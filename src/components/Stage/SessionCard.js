    ...
    const items = !isLoading && props.data.map(event => {
        ...
        if(event.eventBridgeR && event.eventBridgeR.meetingToken) {
            if(userInfo.roles.includes('moderator')) { // this check should be user role for this event
                sessionLink = <a href={`https://meet.nytro.ai/nytro_event/start_meeting.php?user_name=${userInfo.name}&user_email=${userInfo.email}&meeting_token=${event.eventBridgeR.meetingToken}&ret_url=${window.location.origin}`} target="_blank" className="pos-absolute b-20 ">
                    <button className="btn btn-sm pd-x-15 btn-primary tx-12 tx-bold">
                        START SESSION
                    </button>
                </a> 
            } else if (userInfo.roles.includes('panelist')) {
                sessionLink = <a href={`https://meet.nytro.ai/nytro_event/join_meeting.php?user_name=${userInfo.name}&user_email=${userInfo.email}&meeting_token=${event.eventBridgeR.meetingToken}&ret_url=${window.location.origin}`} target="_blank" className="pos-absolute b-20 ">
                    <button className="btn btn-sm pd-x-15 btn-primary tx-12 tx-bold">
                        JOIN SESSION
                    </button>
                </a> 
