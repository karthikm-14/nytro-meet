...
const SessionCard = (props) => {
    ...
    const items = !isLoading && props.data.map(event => {
        ...
        if(event.eventBridgeR && event.eventBridgeR.meetingToken) {
            ...
            } else if (userInfo.roles.includes('panelist') && event.eventBridgeR.bridge.toLocaleLowerCase() === 'zoom' && event.eventBridgeR.eventR.meetingStatus === 'started') {
            ...
            } else if (userInfo.roles.includes('panelist') && event.eventBridgeR.bridge.toLocaleLowerCase() === 'jitsi' && event.eventBridgeR.eventR.meetingStatus === 'started') {
